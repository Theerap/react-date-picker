...
function DatePicker(
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            ...
            if (
                ...
                closeCalendar();
            } else if (calendarRef.current && calendarRef.current.contains(event.target) && !ref.current.mobile) {
    ...
    function openCalendar() {
        ...
        if (!value && !ref.current.date && !range && !multiple) {
            ...
            if ((!minDate || (minDate && date > minDate)) && (!maxDate || (maxDate && date < maxDate))) {
    ...
    function handleChange(date, force, e) {
        ...
        if (date) {
            if (Array.isArray(date)) {
                ...
                setStringDate(
                    date.format(
                        getFormat(timePicker, onlyTimePicker, onlyMonthPicker, onlyYearPicker, format, range, multiple),
...
function getStringDate(date, type, separator, format, formattingIgnoreList) {
    ...
    return type === "button" && date.length > 1 ? [date[0], date[1]].map(toString).join(separator) : date.map(toString).join(separator);
}
